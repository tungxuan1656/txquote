...
struct Provider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
        ...
        let shuffle = listQuotes.shuffled()
        for i in 0...shuffle.count {
            let quote = shuffle[i]
            let date = Calendar.current.date(byAdding: .minute, value: i * 3, to: currentDate)!
            let entry = QuoteEntry(date: date, text: quote.text, link: quote.link)
